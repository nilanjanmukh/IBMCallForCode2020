...
const EditResource = (props) => {
  const clearItem = { userID: userID(), e: 'Help', name: '', description: '', location: '', contact: '', quantity: '1' }
  ...
  const updateItem = () => {
    ...
    update(payload)
      .then(() => {
        Alert.alert('Done', 'Worker has been updated.', [{text: 'OK'}]);
  ...
  const deleteItem = () => {
    ...
    remove(payload)
      .then(() => {
        Alert.alert('Done', 'Worker has been deleted.', [{text: 'OK'}]);
        props.navigation.goBack();
      })
      .catch(err => {
        console.log(err);
  ...
  return (
    <ScrollView style={styles.outerView}>
      ...
      <TextInput
        ...
        returnKeyType='send'
        enablesReturnKeyAutomatically={true}
        placeholder='e.g., Mask making labourers'
      />
